...
class Menu {
  ...
  public function renderMenu($items = [], $level = 1) {
    if ($level == 1) { // к сожалению, не получается присвоить по умолчанию значение переменной класса аргументу метода класса
      $items = $this->menuData; // поэтому приходится внутри метода делать это вручную. либо же делать публичный метод обертку,
    } // который вызывал бы уже внутри класс приватный метод, передавая ему $this->menuData, а приватный метод вызывал бы уже себя рекурсивно
    $html = "<ul>";
